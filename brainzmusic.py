...
class BrainzMusic(dict):
	...
	* *Other tags*:
		...
		def add_artists(cast):
			...
			for artist in cast:
				if isinstance(artist,str):
					artist_tags.append(brush_tag(artist,True))
					continue
				#BUG_OFF(f'440: {artist=}')
				if isinstance(artist,dict) and "name" in artist:
					artist_tags.append(brush_tag(artist["name"],True))
	...
	def lookup_label(S,label,percent=100):
		...
		def lookup_items(label:str,percent:int)->str:
			"""
			The label value that has been seen the most times is 100%
			 the next values that have been "percent" times are added to
			the return list with a "max (= 3)".
			:param label: label to lookup
			:param percent: minimum score under the top scorer to bee added
			:return: a string of label(s) or None
			"""
			if not label in S:
