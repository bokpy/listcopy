...
DEBUGPRINT = print
useless_re=re.compile(r'(?i)Onbekende|Unkown|Various|Unidentified|track.\d+')
...
class TagToken(dict):
	...
	# value='mb_artist,95:3.2f[:1]'
	label  =r'^([^,:[]+)'
	format =r'(:[\df.]+)'
	slice  =r'(\[[^]]+\])'
	intval =r',(\d+)'
	label_value_re = re.compile(label + '|' + slice + '|' + format + '|' + intval )
	# match = label_value_re.findall(value)
	# match = [('mb_artist', '', '', ''), ('', '', '', ',95'), ('', '', ':3.2f', ''), ('', '[:1]', '', '')]
	split_type_value_re = re.compile('([^{]+){([^}]+)}')
	...
	def init_label(S,tokkie_string):
		def value_split(value):
			key_id=['label','slice','format','int']
			# fields = [('mb_artist', '', '', ''), ('', '', '', ',95'), ('', '', ':3.2f', ''), ('', '[:1]', '', '')]
			fields=S.label_value_re.findall(value)
			#BUG_OFF(fields)
			for field in fields:
				for i in range(0,4):
					if field[i]:
		...
		#BUG_OFF(f'\nTagToken.init_label({tokkie_string})')
		type, value = S.split_type_value_re.search(tokkie_string).groups()

		if type == 'literal': # remove quotes and save
			S['fixed'] = re.sub(r"^(['\"])(.*)\1$", r"\2", value)
			return
		...
		if type == 'meaning':
			#DEBUGPRINT(f'245 meaning| "{value=}"')
			S['meaning'] = value
			return
